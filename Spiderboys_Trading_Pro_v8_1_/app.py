...
st.set_page_config(
    page_title="Spiderboys Trading Pro v8.1 Spider AI Workstation",
    page_icon="🕷️",
    layout="wide"
)
...
/* Version 8.1 integrated workstation */
...
<div style="padding:.65rem .25rem .9rem .25rem;">
  ...
  <div style="font-size:.78rem;color:#ffffff;margin-top:.2rem;">Spider AI Workstation v8.1</div>
</div>
""", unsafe_allow_html=True)
...
if page == "Morning Command Center":
    ...
    st.caption("One linked ticker powers the chart, news, trade plan, and journal review.")
    ...
    s1.metric("Active Ticker", active)
    s2.metric("Last", f"${float(last_price):.2f}" if last_price else "—",
              f"{float(day_change):+.2f}%" if day_change is not None else None)
    s3.metric("Spider Score", f'{ai["score"]}/100')
    s4.metric("Trade Grade", ai["grade"])
    s5.metric("Data Mode", payload["chart_mode"])
    ...
    with right:
        ...
        st.markdown(
            ...
            <div class="sb-card">
              ...
              <div style="font-size:2.1rem;font-weight:900;margin:.3rem 0;">{ai["score"]}/100</div>
              <div class="sb-muted">Grade: <b>{ai["grade"]}</b> · Confidence: <b>{ai["confidence"]}</b></div>
              <hr>
              <div><b>Trend:</b> {ai["trend"]}</div>
              <div><b>Entry:</b> {"$"+format(ai["entry"], ".2f") if ai["entry"] else "Not available"}</div>
              <div><b>Stop:</b> {"$"+format(ai["stop"], ".2f") if ai["stop"] else "Not available"}</div>
              <div><b>2R Target:</b> {"$"+format(ai["target_2r"], ".2f") if ai["target_2r"] else "Not available"}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        st.caption("Rule-based setup assessment, not a profit prediction.")
        for reason in ai["reasons"][:5]:
            st.write(f"✓ {reason}")

    st.subheader("Saved Watchlist")
    if "saved_watchlist" not in st.session_state:
        st.session_state["saved_watchlist"] = ["SPY", "QQQ", "AAPL", "NVDA", active]
    ...
    with w2:
        st.caption("Click a ticker button to link it across the app.")

    watch_cols = st.columns(min(5, max(1, len(st.session_state["saved_watchlist"]))))
    for idx, symbol in enumerate(st.session_state["saved_watchlist"]):
        with watch_cols[idx % len(watch_cols)]:
            if st.button(symbol, key=f"watch_{symbol}_{idx}", use_container_width=True):
                set_active_ticker(symbol)
                st.rerun()

    if isinstance(payload.get("news"), pd.DataFrame) and not payload["news"].empty:
        st.subheader("Latest Company News")
        news_view = payload["news"].head(8).copy()
        cols = [c for c in ["published", "headline", "source", "url"] if c in news_view.columns]
        st.dataframe(
            news_view[cols],
