...
st.set_page_config(
    page_title="Spiderboys Trading Pro v11 Scanner Terminal",
    page_icon="🕷️",
    layout="wide"
)
...
def build_training_scanner_frame(market_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result["data_source"] = "Training metrics"
    return result
...
def scanner_view_frame(scanner_name: str) -> pd.DataFrame:
    ...
    if scanner_name == "HOD Momentum":
        frame = frame.sort_values(
            ["hod_distance_pct", "spider_score", "rvol_5m"],
            ascending=[True, False, False],
        )
    elif scanner_name == "Top Gappers":
        frame = frame.sort_values(["gap_pct", "spider_score"], ascending=[False, False])
    elif scanner_name == "Volume Surge":
        frame = frame.sort_values(["rvol_5m", "relative_volume"], ascending=[False, False])
    elif scanner_name == "Low Float":
        frame = frame[frame["float_m"] <= 20].sort_values(
            ["spider_score", "relative_volume"], ascending=[False, False]
        )
...
def scanner_display_frame(frame: pd.DataFrame, compact: bool = False) -> pd.DataFrame:
    columns = [
        "alert_time", "symbol_news", "price", "volume", "float_m",
        "relative_volume", "rvol_5m", "gap_pct", "day_change_pct",
        "change_5m_pct", "hod_distance_pct", "spider_score", "setup_status",
    ]
    if compact:
        columns = [
            "alert_time", "symbol_news", "price", "float_m",
            "relative_volume", "gap_pct", "spider_score",
        ]
...
def style_scanner_table(display: pd.DataFrame):
    ...
    if "Spider Score" in display.columns:
        styler = styler.background_gradient(subset=["Spider Score"], cmap="RdYlGn", vmin=0, vmax=100)
...
<div style="padding:.65rem .25rem .9rem .25rem;">
  ...
  <div style="font-size:.78rem;color:#ffffff;margin-top:.2rem;">Scanner Terminal v11.0</div>
</div>
""", unsafe_allow_html=True)
...
if page == "Morning Command Center":
    ...
    with tool_b:
        scanner_name = st.selectbox(
            "Scanner",
            ["HOD Momentum", "Top Gappers", "Volume Surge", "Low Float", "News Catalysts"],
            index=["HOD Momentum", "Top Gappers", "Volume Surge", "Low Float", "News Catalysts"].index(
    ...
    with tool_c:
        timeframe = st.segmented_control(
            ...
            default="5min",
            label_visibility="collapsed",
            key="v11_primary_timeframe",
        )

    with tool_d:
        if st.session_state["right_panel_open"]:
            if st.button("Collapse Intel ▶", use_container_width=True, key="v11_hide_intel"):
                st.session_state["right_panel_open"] = False
                st.rerun()
        else:
            if st.button("Restore Intel ◀", type="primary", use_container_width=True, key="v11_show_intel"):
                st.session_state["right_panel_open"] = True
                st.rerun()
    ...
    with dock_scanner:
        ...
        with sub2:
            ...
            render_scanner_terminal("Volume Surge", compact=True, height=330, key="dock_volume")
    ...
    **Lesson:** {trade["lesson"]}
    ...
    st.title("Spider Momentum Scanner")
    st.caption("Professional scanner workspace with selectable views, dense color coding, news flags, and linked ticker analysis.")

    top_a, top_b, top_c, top_d = st.columns([2, 1.1, 1.1, 1.1])
    with top_a:
        scanner_name = st.selectbox(
            "Select Scanner",
            ["HOD Momentum", "Top Gappers", "Volume Surge", "Low Float", "News Catalysts"],
    ...
    with top_c:
        max_float = st.number_input("Max Float M", min_value=1.0, value=100.0, step=5.0)
    with top_d:
    ...
    frame = frame[
        ...
        & (pd.to_numeric(frame["float_m"], errors="coerce") <= max_float)
        & (pd.to_numeric(frame["spider_score"], errors="coerce") >= min_score)
    ].reset_index(drop=True)

    st.markdown(
        f"""
        <div class="scanner-status-row">
          <span>SCANNER: {scanner_name.upper()}</span>
          <span>ROWS: {len(frame)}</span>
          <span>MODE: TRAINING METRICS + LIVE TICKER LINK</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    event = st.dataframe(
        ...
        on_select="rerun",
        selection_mode="single-row",
        key=f"full_scanner_{scanner_name}",
    )
    ...
    if selected_symbol:
        st.success(f"{selected_symbol} is linked to Charts, News, Trade Plan, and the Command Center.")
        ...
        info_col, chart_col, ai_col = st.columns([1.15, 2.2, 1.15], gap="small")
        with info_col:
            st.markdown('<div class="terminal-panel-title">STOCK QUOTE</div>', unsafe_allow_html=True)
            st.metric(
                selected_symbol,
                f'${float(quote.get("price")):.2f}' if quote.get("price") else "—",
                f'{float(quote.get("change_pct", 0)):+.2f}%' if quote else None,
            ...
            if profile.get("shares_outstanding_m"):
                st.write(f"**Shares Out:** {float(profile['shares_outstanding_m']):,.2f}M")

            news_view = compact_news_view(payload.get("news"), 4)
            st.markdown('<div class="terminal-panel-title">NEWS</div>', unsafe_allow_html=True)
            if news_view.empty:
                st.caption("No recent company news returned.")
            else:
                for _, item in news_view.iterrows():
        ...
        with chart_col:
            st.markdown('<div class="terminal-panel-title">MULTI-TIMEFRAME CHARTS</div>', unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1:
                mini_candlestick_chart(selected_symbol, "5min", 350, "scanner_5m")
            with c2:
                mini_candlestick_chart(selected_symbol, "1min", 350, "scanner_1m")
            mini_candlestick_chart(selected_symbol, "15min", 310, "scanner_15m")
            ...
            st.markdown(
                ...
                <div class="sb-card ai-panel">
                  ...
                  <div class="level-grid one-col">
                    ...
                    <div><small>Entry</small><b>{"$"+format(ai["entry"], ".2f") if ai["entry"] else "—"}</b></div>
                    <div><small>Stop</small><b>{"$"+format(ai["stop"], ".2f") if ai["stop"] else "—"}</b></div>
                    <div><small>2R</small><b>{"$"+format(ai["target_2r"], ".2f") if ai["target_2r"] else "—"}</b></div>
                  </div>
                </div>
                """,
                unsafe_allow_html=True,
