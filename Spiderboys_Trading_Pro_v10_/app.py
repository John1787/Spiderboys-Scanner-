...
st.set_page_config(
    page_title="Spiderboys Trading Pro v10 Flagship Workstation",
    page_icon="🕷️",
    layout="wide"
)
...
<div style="padding:.65rem .25rem .9rem .25rem;">
  ...
  <div style="font-size:.78rem;color:#ffffff;margin-top:.2rem;">Flagship Workstation v10.0</div>
</div>
""", unsafe_allow_html=True)
...
if page == "Morning Command Center":
    ...
    st.markdown(
        ...
        <div class="workspace-title">
          ...
          <div class="workspace-heading">Flagship Command Center</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    k5.metric("Data", payload["chart_mode"])

    b1, tf_col, b2 = st.columns([1.25, 4, 1.25])
    with b1:
        if st.session_state["scanner_panel_open"]:
            if st.button("◀ Collapse Scanner", use_container_width=True):
                st.session_state["scanner_panel_open"] = False
                st.rerun()
        else:
            if st.button("▶ Restore Scanner", type="primary", use_container_width=True):
                st.session_state["scanner_panel_open"] = True
                st.rerun()
    with tf_col:
    ...
    with b2:
        if st.session_state["right_panel_open"]:
            if st.button("Collapse AI ▶", use_container_width=True):
                st.session_state["right_panel_open"] = False
                st.rerun()
        else:
            if st.button("Restore AI ◀", type="primary", use_container_width=True):
                st.session_state["right_panel_open"] = True
                st.rerun()
    ...
    if layout_mode == "Compact":
        st.subheader(f"{active} Focus Chart")
        candlestick_chart(active, 540, interval=timeframe)
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(
                f"""
                <div class="sb-card ai-panel">
                  <div class="sb-card-title">{active} Spider AI</div>
        ...
        if scanner_open and right_open:
            scanner_col, chart_col, ai_col = st.columns([1.05, 2.15, 1.05], gap="small")
        elif scanner_open:
            scanner_col, chart_col = st.columns([1.05, 3.2], gap="small")
            ai_col = None
        elif right_open:
            chart_col, ai_col = st.columns([3.2, 1.05], gap="small")
            scanner_col = None
        else:
            chart_col = st.container()
            scanner_col = ai_col = None

        if scanner_col is not None:
            with scanner_col:
                st.markdown('<div class="panel-label">MOMENTUM SCANNER</div>', unsafe_allow_html=True)
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "price": st.column_config.NumberColumn("Last", format="$%.2f"),
                        "change_pct": st.column_config.NumberColumn("Day %", format="%.2f%%"),
                        "high": None,
                        "low": None,
                    },
                )
                st.markdown('<div class="panel-label">QUICK SYMBOLS</div>', unsafe_allow_html=True)
                symbols = st.session_state["saved_watchlist"][:8]
                for n in range(0, len(symbols), 2):
                    cc = st.columns(2)
                    for j, symbol in enumerate(symbols[n:n+2]):
                        with cc[j]:
                            if st.button(symbol, key=f"quick_{symbol}_{n}", use_container_width=True):
                                set_active_ticker(symbol)
                                st.rerun()

        with chart_col:
            st.markdown(f'<div class="panel-label">PRIMARY CHART · {active} · {timeframe}</div>', unsafe_allow_html=True)
            candlestick_chart(active, 620, interval=timeframe)
            q1, q2, q3, q4 = st.columns(4)
            q1.metric("Last", f'${float(quote.get("price")):.2f}' if quote.get("price") else "—")
            q2.metric("Day High", f'${float(quote.get("high")):.2f}' if quote.get("high") else "—")
            q3.metric("Day Low", f'${float(quote.get("low")):.2f}' if quote.get("low") else "—")
            q4.metric("Grade", ai["grade"])

        if ai_col is not None:
            with ai_col:
                st.markdown('<div class="panel-label">SPIDER AI</div>', unsafe_allow_html=True)
                st.markdown(
                    f"""
                    <div class="sb-card ai-panel">
    ...
    tabs = st.tabs(["Scanner", "Trade Plan", "News", "Journal", "Performance", "Diagnostics"])
    with tabs[0]:
        watch = build_watchlist_snapshot(st.session_state["saved_watchlist"])
        st.dataframe(watch, use_container_width=True, hide_index=True)
    with tabs[1]:
        d1, d2, d3, d4 = st.columns(4)
        d1.metric("Entry", "$"+format(ai["entry"], ".2f") if ai["entry"] else "—")
        d2.metric("Stop", "$"+format(ai["stop"], ".2f") if ai["stop"] else "—")
        d3.metric("2R Target", "$"+format(ai["target_2r"], ".2f") if ai["target_2r"] else "—")
        d4.metric("Grade", ai["grade"])
    with tabs[2]:
        news_small = compact_news_view(payload.get("news"), 10)
        if news_small.empty:
            st.info("No recent company news returned.")
        else:
            st.dataframe(news_small, use_container_width=True, hide_index=True,
                         column_config={"url": st.column_config.LinkColumn("Article")})
    with tabs[3]:
        st.dataframe(journal.tail(8), use_container_width=True, hide_index=True)
    with tabs[4]:
        p1, p2, p3 = st.columns(3)
        p1.metric("Trades", len(journal))
        p2.metric("Win Rate", f"{journal['win'].mean()*100:.1f}%" if len(journal) else "—")
        p3.metric("Net P/L", f"${journal['pnl'].sum():,.2f}" if len(journal) else "—")
    with tabs[5]:
