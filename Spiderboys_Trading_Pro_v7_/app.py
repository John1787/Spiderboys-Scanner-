...
st.set_page_config(
    page_title="Spiderboys Trading Pro v7 Integrated Workstation",
    page_icon="🕷️",
    layout="wide"
)
...
/* Version 7 integrated workstation */
...
def get_chart_frame(symbol: str, interval: str = "5min"):
    """Return chart data plus a plain-language source label."""
    symbol = normalize_symbol(symbol)
    if FMP_KEY:
        try:
            bars = load_live_chart(symbol, interval, FMP_KEY)
            return bars, "LIVE", "FMP intraday bars"
        except MarketDataError as exc:
            live_error = str(exc)
        except Exception as exc:
            live_error = f"Unexpected live-chart error: {exc}"
    else:
        live_error = "FMP key is not configured."

    demo = market[market["ticker"] == symbol].sort_values("datetime").copy()
    if not demo.empty:
        return demo, "DEMO", f"Training chart fallback — {live_error}"
    return pd.DataFrame(), "UNAVAILABLE", live_error
...
<div style="padding:.65rem .25rem .9rem .25rem;">
  ...
  <div style="font-size:.78rem;color:#91a9c2;margin-top:.2rem;">Integrated Workstation v7.0</div>
</div>
""", unsafe_allow_html=True)
...
<div class="sb-hero sb-hero-compact">
  ...
  <div class="sb-hero-sub">Scanner · news · charts · planning · journal</div>
  <div class="sb-status-row">
    <span class="sb-chip sb-chip-live">● Live data enabled</span>
...
if page == "Morning Command Center":
    ...
    st.caption(f"Your linked workspace ticker is **{active}**. Change it in the sidebar or select a Scanner row.")

    if FINNHUB_KEY:
        try:
            live_quote = finnhub_quote(active, FINNHUB_KEY)
            q1, q2, q3, q4 = st.columns(4)
            q1.metric(f"{active} Last", f'${live_quote["price"]:.2f}', f'{live_quote["change_pct"]:+.2f}%')
            q2.metric("Day High", f'${live_quote["high"]:.2f}')
            q3.metric("Day Low", f'${live_quote["low"]:.2f}')
            q4.metric("Previous Close", f'${live_quote["previous_close"]:.2f}')
        except MarketDataError as exc:
            st.caption(f"Live quote unavailable: {exc}")
    ...
    ticker = st.session_state["active_ticker"]
    bars, chart_mode = get_chart_frame(ticker, "5min")
    if bars.empty:
        ...
        default_entry = float(last["high"])
        default_stop = float(recent["low"].min())
        ...
        with right:
            ...
            entry = st.number_input("Planned Entry", min_value=0.01, value=round(default_entry, 2), step=0.01)
            stop = st.number_input("Planned Stop", min_value=0.01, value=round(default_stop, 2), step=0.01)
        ...
        if grade == "A":
            st.success("The written plan is complete. Wait for the actual trigger and honor the stop.")
        else:
            st.warning("The plan is incomplete. Resolve the unchecked items before considering an entry.")
