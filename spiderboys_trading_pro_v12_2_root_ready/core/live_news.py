...
def fmp_daily_history(symbol: str, api_key: str, lookback_days: int = 45) -> pd.DataFrame:
    """Retrieve recent end-of-day history, preferring FMP's lighter chart endpoint."""
    symbol = symbol.upper().strip()
    end = date.today()
    start = end - timedelta(days=max(int(lookback_days), 15))
    errors: list[str] = []

    for endpoint in ("historical-price-eod/light", "historical-price-eod/full"):
        try:
            payload = _get_json(
                f"{FMP_BASE}/{endpoint}",
                {
            ...
            if not rows:
                errors.append(f"{endpoint}: no rows")
                continue

            frame = pd.DataFrame(rows).copy()
            if "date" not in frame.columns:
                errors.append(f"{endpoint}: missing date")
                continue

            frame["date"] = pd.to_datetime(frame["date"], errors="coerce")
            frame = frame.dropna(subset=["date"]).sort_values("date").drop_duplicates("date")

            # Light endpoint generally returns date, price, volume. Full returns OHLCV.
            ...
            for col in ["open", "high", "low", "close", "volume"]:
                frame[col] = pd.to_numeric(frame[col], errors="coerce")
            frame = frame.dropna(subset=["close"]).tail(40).reset_index(drop=True)
            if frame.empty:
                errors.append(f"{endpoint}: no usable rows")
                continue
            ...
            frame["source"] = f"FMP {endpoint.split('/')[-1].title()} EOD"
            frame["ticker"] = symbol
            return frame
        except MarketDataError as exc:
            errors.append(f"{endpoint}: {exc}")

    raise MarketDataError("Daily history unavailable. " + " | ".join(errors[-2:]))
