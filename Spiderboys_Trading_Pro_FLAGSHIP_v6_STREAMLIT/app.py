...
st.set_page_config(page_title="Spiderboys Trading Pro Flagship", page_icon="🕷️", layout="wide", initial_sidebar_state="expanded")
st.markdown("""
<style>
.block-container{padding-top:1rem;max-width:1650px}.hero{padding:22px;border:1px solid #29384a;border-radius:18px;background:linear-gradient(135deg,#111b27,#0a1017 65%,#26170b);margin-bottom:14px}.hero h1{margin:0;font-size:2.05rem}.hero p{margin:.35rem 0 0;color:#aab7c7}.eyebrow{font-size:.76rem;letter-spacing:.11em;color:#eca54b}.panel{border:1px solid #29384a;border-radius:14px;padding:14px;background:#101720}.good{color:#43d17b}.warn{color:#f5b942}.bad{color:#ff647c}.small{font-size:.82rem;color:#98a8bb}[data-testid="stMetricValue"]{font-size:1.48rem}
...
with st.sidebar:
    st.title("🕷️ Spiderboys")
    st.caption("Trading Pro • Flagship v6")
    page=st.radio("Workspace",["Command Center","Elite Scanner","Charts & TradingView","Trade Planner","Risk Command","Trading Journal","Performance","Replay Academy","AI Coach","TradingView Radar","Settings & Backup"])
    st.divider()
...
st.markdown('<div class="hero"><div class="eyebrow">FLAGSHIP STREAMLIT WORKSTATION</div><h1>Spiderboys Trading Pro</h1><p>Scan • Rank • Plan • Control Risk • Journal • Improve</p></div>',unsafe_allow_html=True)
...
def draw_chart(ticker,height=520,reveal=None):
    ...
    fig.add_trace(go.Scatter(x=g.datetime,y=g.vwap,name="VWAP",line=dict(width=2)))
    fig.add_trace(go.Scatter(x=g.datetime,y=g.ema9,name="EMA 9",line=dict(width=1)))
    fig.add_trace(go.Scatter(x=g.datetime,y=g.ema20,name="EMA 20",line=dict(width=1)))
    ...
    fig.update_layout(height=height,margin=dict(l=8,r=8,t=30,b=8),xaxis_rangeslider_visible=False,legend_orientation="h",paper_bgcolor="#101720",plot_bgcolor="#101720",font=dict(color="#dce6f2"))
...
if page=="Command Center":
    ...
    st.title("Elite Momentum Scanner")
    ...
    st.title("Risk-First Trade Planner")
    a,b=st.columns(2)
    with a:
    ...
    st.title("Risk Command Center"); max_loss=account*daily_loss_pct/100
    ...
    st.title("Performance Analytics"); journal=load_journal(BASE_DIR); stats=summarize_journal(journal); c=st.columns(6)
    ...
    st.title("Replay Academy"); ticker=st.selectbox("Replay ticker",sorted(market.ticker.unique())); g=market[market.ticker==ticker].sort_values("datetime"); reveal=st.slider("Reveal candles",10,max(10,len(g)-1),min(25,max(10,len(g)-1))); draw_chart(ticker,530,reveal)
    ...
    st.title("Execution Coach"); c=st.columns(4); ticker=c[0].text_input("Ticker","SOUN",key="coach_ticker"); setup=c[1].selectbox("Setup",["First Pullback","HOD Break","ORB","VWAP Reclaim"],key="coach_setup"); emotion=c[2].selectbox("State",["Calm","Focused","FOMO","Revenge","Tired"],key="coach_emotion"); shares=c[3].number_input("Shares",1,100000,500,key="coach_shares")
